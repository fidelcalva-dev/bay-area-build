...
import {
  ...
  Calendar, Lock, Shield, Phone, AlertCircle, AlertTriangle, ShieldAlert, RefreshCw,
} from 'lucide-react';
...
export default function PortalPay() {
  ...
  useEffect(() => {
    if (orderInfo && fullAmount <= 0 && !isLoading) {
      console.log('[PortalPay] fullAmount <= 0, auto-skipping payment step');
      if (orderId) navigate(`/portal/order/${orderId}`);
      else navigate('/portal');
    }
  }, [orderInfo, fullAmount, orderId, navigate, isLoading]);

  // Diagnostic logging
  useEffect(() => {
    if (orderInfo) {
      console.log('[PortalPay] Diagnostic:', {
        orderId,
        fullAmount,
        depositAmount,
  ...
  const handlePay = async (type: 'deposit' | 'balance') => {
    ...
    try {
      ...
      console.error('[PortalPay] Payment session error:', err);

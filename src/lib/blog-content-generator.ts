...
const CATEGORY_CONTENT: Record<string, Record<string, (topic: SeoBlogTopic) => string>> = {
  ...
  materials: {
    'flat-fee pricing': () =>
      `Heavy material dumpsters — available in 6, 8, and 10 yard sizes — use flat-fee pricing. This means the quoted price includes delivery, pickup, rental period, AND disposal, with no per-ton overage charges.\n\nThis pricing model exists because heavy materials like concrete, dirt, and rock are predictably dense. A 6-yard dumpster of concrete weighs roughly 8 to 12 tons. Under standard per-ton pricing at $${PRICING_POLICIES.overagePerTonGeneral}/ton overage, the disposal cost alone would far exceed the flat fee.\n\nFlat-fee pricing benefits the customer by providing cost certainty and benefits the operator by ensuring loads go to appropriate recycling facilities rather than mixed-waste landfills.\n\nThe trade-off: flat-fee containers must contain ONLY the specified heavy material. Any contamination with general debris (wood, plastic, drywall, household trash) triggers reclassification to general debris rates.`,

    ...
    try {
      ...
      const [quotesRes, contractsRes, paymentsRes] = await Promise.all([
        supabase.from('quotes')
          .select('id, customer_name, customer_phone, status, subtotal, created_at, material_type, zip_code, user_selected_size_yards, recommended_size_yards, preferred_delivery_date')
          .in('status', ['saved', 'pending', 'sent'])
          .order('created_at', { ascending: false })
          .limit(200),
        supabase.from('quote_contracts')
          .select('id, customer_name, status, sent_at, created_at, quote_id')
          .in('status', ['pending', 'sent'])
          .order('created_at', { ascending: false })
          .limit(100),
        (supabase.from('payment_requests' as any) as any)
          .select('id, status, amount, created_at, customer_id')
      ...
      const missingSize = quotes.filter((q: any) => !q.user_selected_size_yards && !q.recommended_size_yards);
      const missingDelivery = quotes.filter((q: any) => !q.preferred_delivery_date);
      const readyToSend = quotes.filter((q: any) =>
        q.status === 'saved' && q.customer_name && q.customer_phone && q.subtotal &&
        (q.user_selected_size_yards || q.recommended_size_yards)
      );
      const readyToSchedule = quotes.filter((q: any) =>
        q.customer_name && q.customer_phone && q.subtotal &&
        (q.user_selected_size_yards || q.recommended_size_yards) &&
        q.preferred_delivery_date
      );
      ...
      // Quotes missing critical info but old
      missingSize
        .filter((q: any) => q.created_at < twoDaysAgo)
        .slice(0, 3)
        .forEach((q: any) => {
          stuckDeals.push({
            id: q.id,
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <ReadinessItem icon={<FileText className="w-4 h-4 text-primary" />} label="Ready to Send" value={stats.quotesReadyToSend} link="/sales/quotes" />
            <ReadinessItem icon={<Truck className="w-4 h-4 text-primary" />} label="Ready to Schedule" value={stats.quotesReadyToSchedule} link="/sales/quotes" />
            <ReadinessItem icon={<ScrollText className="w-4 h-4 text-primary" />} label="Waiting on Contract" value={stats.quotesWaitingContract} link="/sales/quotes" alert={stats.quotesWaitingContract > 0} />
            <ReadinessItem icon={<CreditCard className="w-4 h-4 text-primary" />} label="Waiting on Payment" value={stats.quotesWaitingPayment} link="/finance/payments" alert={stats.quotesWaitingPayment > 0} />
            <ReadinessItem icon={<AlertTriangle className="w-4 h-4 text-destructive" />} label="Missing Size" value={stats.quotesMissingSize} link="/sales/quotes" alert={stats.quotesMissingSize > 0} />
            <ReadinessItem icon={<Clock className="w-4 h-4 text-muted-foreground" />} label="Missing Delivery" value={stats.quotesMissingDelivery} link="/sales/quotes" />
          </div>
        </CardContent>
      </Card>
...
function ReadinessItem({ icon, label, value, link, alert }: {
  ...
  return (
    <Link
      to={link}
      className={`flex items-center gap-2.5 p-3 rounded-lg border transition-colors hover:bg-muted/50 ${alert ? 'border-destructive/30 bg-destructive/5' : ''}`}
    >
      {icon}

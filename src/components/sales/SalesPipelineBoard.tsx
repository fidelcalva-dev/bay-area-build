...
import { Phone, MessageSquare, FileText, Clock } from "lucide-react";
...
const PIPELINE_STAGES = [
  ...
  { key: "payment_received", label: "Paid", color: "bg-cyan-500" },
  { key: "order_created", label: "Scheduled", color: "bg-lime-500" },
];
...
export default function SalesPipelineBoard({ leads }: Props) {
  const navigate = useNavigate();
  const now = new Date();

  const stageLeads = (stageKey: string) => {
    // Map "quoted" to "quote_sent" for backwards compat
    return leads.filter(l => {
      const status = l.lead_status;
      if (stageKey === "quote_sent") return status === "quote_sent" || status === "quoted";

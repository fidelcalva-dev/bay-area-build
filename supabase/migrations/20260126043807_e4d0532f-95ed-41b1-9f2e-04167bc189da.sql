-- Fix: entity_id is UUID, not text
...
BEGIN
  ...
  IF v_invoice_id IS NULL THEN
    v_invoice_number := 'INV-' || to_char(now(), 'YYYYMMDD') || '-' || SUBSTRING(p_order_id::text, 1, 8);
    
    INSERT INTO public.invoices (order_id, invoice_number, amount_due, balance_due, payment_status)
    VALUES (p_order_id, v_invoice_number, v_new_total, v_new_total, 'unpaid')
  ...
  IF v_price_delta > v_approval_threshold THEN
    INSERT INTO public.approval_requests (
      request_type, entity_type, entity_id, requested_by, requested_value, reason, status
    ) VALUES (
